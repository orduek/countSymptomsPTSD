...
```{r,include = FALSE }
datax <- read_delim("datax.csv", ";", escape_double = FALSE, 
    trim_ws = TRUE)
```
1. PCA
<br />
...
iteratePCA <- function(data, n_itr, sample_size, freqCUT) {
  ...
  for (i in 1:n_itr) {
    ...
    items.parallel_top <- fa.parallel(dataSAMP_top_cor, fa = "pc", n.obs = nrow(dataSAMP_top))
    items.parallel_low <- fa.parallel(dataSAMP_low_cor, fa = "pc", n.obs = nrow(dataSAMP_low))
...
<br />
```{r}
model_3Factor <- ' EE =~ mbi_1+mbi_2+mbi_3+mbi_4+mbi_6
                   PA =~ mbi_5+mbi_7+mbi_10+mbi_11+mbi_12+mbi_16
                   DP =~ mbi_13+mbi_14+mbi_15+mbi_8+mbi_9'
```

```{r, include = FALSE}
model_Factor <- model_3Factor
...
<br />
```{r}
size_n <- c(250, 500, 750, 1000, 1500, 2000, 5000)
nitr <- 50
